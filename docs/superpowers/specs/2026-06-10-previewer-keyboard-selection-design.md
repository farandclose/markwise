...
2. _Inline caret element (rejected):_ `range.insertNode` of a caret span, as the insert compose does for its field. A compose box is one-shot; a caret moves every keystroke - constant text-node splitting and healing would be slow and fragile.
  
3. _Secretly editable document (rejected):_ `contenteditable` would give a free native caret and native keyboard nav, but requires intercepting every mutation vector (typing, paste, drop, undo, IME, autocorrect) forever. One miss silently edits the document - unacceptable against "the column equals the file".

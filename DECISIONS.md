...
* * *
### D43 - Preview writes carry a version precondition; the server gates Host and content-type (2026-06-10)
The previewer's write path now refuses to apply a mutation built against content that has changed. `GET /api/doc` returns `version` (the `shortHash` of the exact file bytes the payload was built from); every `POST` must echo it in an `x-mw-version` header. The single `persist()` write path re-reads the file and rejects with `409` on mismatch (`428` when the header is missing entirely). The browser repaints from the server and asks the reviewer to redo the action; it also quietly revalidates on window focus (repainting only when the version moved and no draft/compose is open).

**Why:** the product's own loop invites the race - an agent revises the file while the reviewer's tab still shows the old render. Offsets from a stale tab would still be "in range" and would silently anchor a note to the wrong text, the worst failure mode because it looks like success. The fingerprint turns that into a visible, recoverable refusal. This is the offset-based create API's missing optimistic-concurrency token, and it doubles as CSRF protection: custom headers cannot cross origins without a CORS preflight this server never answers.

Two further gates on the same surface, closing the "any local page can poke localhost" hole: every request's `Host` header must be a loopback name (DNS-rebinding refusal, `403`), and a non-empty POST body must be declared `application/json` (`415` - the no-cors `text/plain` CSRF shape).

Companions in the same pass: `prepare` script so `npm i github:...` builds `dist/` on install (the documented install was broken without it); `stripText` rebuilt on `parse()` so fenced code blocks demonstrating `mw:` syntax survive `export`; a Playwright smoke suite (`test/e2e/`, `npm run test:e2e`) covering the previewer gestures the unit suite cannot reach (select->comment, select+type->replace, click+type->insert, Delete->suggest-delete, resolve, discard, and the stale-tab 409 recovery). Builds on D40/D42.
